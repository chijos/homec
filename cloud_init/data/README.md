...
- [http://cloud-init-server:8080/cloud-init/user-data](http://cloud-init-server:8080/cloud-init/user-data)
  Copy the user-data.example file into a user-data file (ignored by git already). See cloud-init reference documentation for what can be added in here: user creation, installing SSH keys, running a
...
- [http://cloud-init-server:8080/cloud-init/vendor-data](http://cloud-init-server:8080/cloud-init/vendor-data)
  Usually supplied by the cloud vendor (think AWS, GCP, Azure, etc.). I'm not intending to use it here, so it just has the required `#cloud-config` header. The file needs to exist even if it is no
  The file needs to exist even if it is not used. See cloud-init reference documentation for more information.
